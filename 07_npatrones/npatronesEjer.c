...
static void mainLoop(void) {
  ...
  for (i=0; i<nobjects; i++) {
    ...
    if(k != -1) {   // Si ha detectado el patron en algun sitio...
      if (max<marker_info[k].cf) {
        max=marker_info[k].cf;
        id_selected=i;
      }
      objects[i].visible = 1;
    ...
    printf("El objeto %d tiene confianza %f\n",i,marker_info[k].cf );
  }
printf("El objeto seleccionado es %d tiene confianza %f\n",id_selected,max );
  for (i=0; i<nobjects; i++) {
    for(j = 0, k = -1; j < marker_num; j++) {
      if(objects[i].id == marker_info[j].id) {
  if (k == -1) k = j;
    ...
    if(k != -1&&i==id_selected) {   // Si ha detectado el patron en algun sitio...
      objects[i].visible = 1;
      arGetTransMat(&marker_info[k], objects[i].center,
        objects[i].width, objects[i].patt_trans);
    } else { objects[i].visible = 0; }  // El objeto no es visible
